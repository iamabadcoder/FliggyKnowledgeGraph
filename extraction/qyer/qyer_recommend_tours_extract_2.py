	...
	if '1日' in tour_name_line or '一日' in tour_name_line:  # 一日游线路解析
		...
		for i in range(line_num + 1, 2000):
			...
			if i_line.startswith('## ') or i_line.startswith('# '):
				...
				print i_line
	# 写如文件
	file_object = open('qyer_tours_2.txt', 'a+')
	if len(all_day) >= 1:
...
def extract_tours(dir_path):
	...
	for afile in file_list:
		if 'html.md' in afile and file_num <= 200:
			...
			count_line = len(open(file_path, 'rU').readlines())
			for i in range(1, count_line + 1):
				i_line = read_target_line(file_path, i)
				if '# ___op___推荐路线' in i_line and '1日' in i_line:
					next_line = read_target_line(file_path, i + 1)
					file_num += 1
					if '图：' not in next_line:
						print next_line
					else:
						print read_target_line(file_path, i + 2)
	print file_num
...
if __name__ == '__main__':
	...
	dir_path = '/Users/caolei/Downloads/qyer/qyer_md/part_2'
