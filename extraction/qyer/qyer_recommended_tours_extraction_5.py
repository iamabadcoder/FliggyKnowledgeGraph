...
def get_tour_line_desc(file_path, ln, pound_sign_count):
	tour_desc = '####'
...
def extract_tour(file_path, ln, destination):
	...
	tour_name = unicode(tour_name)
	pound_sign_count = tour_name.count('#')

	tour_desc = get_tour_line_desc(file_path, ln, pound_sign_count)
	tour_desc = unicode(tour_desc)
	all_day = {}
	if len(re.compile(ur'(\d+)日').findall(unicode(tour_name))) > 0:  # n日游
		day_list = re.compile(ur'(\d+)日').findall(unicode(tour_name))
		if int(day_list[0]) == 1:  # 1日游
			...
			if int(re.compile(ur'(\d+)日').findall(tour_name)[0]) == len(tour_desc_by_day) - 1:
	...
	if len(all_day) > 0:
		write_to_file('qyer_recommended_tours_5.txt', destination, clean_line(tour_name), all_day)
